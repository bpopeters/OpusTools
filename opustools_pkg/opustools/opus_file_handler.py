...
class OpusFileHandler:
    ...
    def __init__(
            self, download_dir, source_zip, target_zip, directory,
    ...
    def download_files(self):
        ...
        arguments = {
            'source': self.fromto[0],
            'target': self.fromto[1],
        ...
        if os.path.isfile(align_name):
            ...
            if os.path.isfile(local_align_name):
                ...
                raise FileNotFoundError(
                    'No alignment file "{default}" or "{downloaded}" found'.format(
                        default=align_name, downloaded=local_align_name)
                    )
    ...
    def open_specific_zips(self, src_zip_name, trg_zip_name):
        if self.verbose:
            print('Opening zip archive "{}" ... '.format(src_zip_name), end='')
        src_zip = zipfile.ZipFile(src_zip_name, 'r')
        if self.verbose:
            print('Done')
            print('Opening zip archive "{}" ... '.format(trg_zip_name), end='')
    ...
    def _zip_paths(self):
        if os.path.isfile(self.src_zip_name) and os.path.isfile(self.trg_zip_name):
            return self.src_zip_name, self.trg_zip_name

        src, trg = self.fromto
        pattern = "_".join(
            [self.directory, self.release, self.preprocess, "{}.zip"]
        )
        local_src_name = os.path.join(self.download_dir, pattern.format(src))
        local_trg_name = os.path.join(self.download_dir, pattern.format(trg))
        if os.path.isfile(local_src_name) and os.path.isfile(local_trg_name):
            return local_src_name, local_trg_name

        print('No zip files found.')
        self.download_files()
        if os.path.isfile(local_src_name) and os.path.isfile(local_trg_name):
            return local_src_name, local_trg_name
        msg = 'No zip files "{}" and "{}" OR "{}" and "{}" found'.format(
            self.src_zip_name, self.trg_zip_name, local_src_name, local_trg_name
        )
    ...
    def open_zipfiles(self):
        ...
        missing files"""
        src_zip_path, trg_zip_path = self._zip_paths()
        self.src_zip, self.trg_zip = self.open_specific_zips(src_zip_path, trg_zip_path)
    ...
    def open_sentence_file(self, doc_name, direction):
        ...
        opus_doc_name = os.path.join(
            self.directory, self.preprocess, doc_name[:-3]
        )

        if self.verbose:
            print('Reading {}_file "{}"'.format(direction, opus_doc_name))
